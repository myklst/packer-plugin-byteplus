...
func WaitSgStatus(stateBag multistep.StateBag, sgId, status string) (*vpc.DescribeSecurityGroupsOutput, error) {
	client := stateBag.Get("client").(*VolcengineClientWrapper)
	out, err := WaitFor(&WaitForParam{
		Call: func() (interface{}, error) {
			...
			return client.VpcClient.DescribeSecurityGroups(&input)
		},
		Process: func(i interface{}, err error) ProcessResult {
			if err != nil {
				return WaitForRetry
			}
			output := i.(*vpc.DescribeSecurityGroupsOutput)
			if len(output.SecurityGroups) < 1 {
				return WaitForRetry
			}
			if *output.SecurityGroups[0].Status == status {
				return WaitForSuccess
			} else {
				return WaitForRetry
			}
	...
	if out != nil {
		return out.(*vpc.DescribeSecurityGroupsOutput), err
	}
	return nil, err
}
